...
*}
<h3>{l s='Credenciales' mod='pagomio'}</h3>
<form action="{$formCredential|escape:'htmlall':'UTF-8'}" method="POST">
	<input type="hidden" name="submitPagomio" value="1" />
	{foreach from=$credentialInputVar item=input}
		{if $input.type == 'text'}
			<ul>
				<li><label class="label_payu">{$input.label|escape:'htmlall':'UTF-8'}</label></li>
				<li><input class="full input_pagomio" type="{$input.type|escape:'htmlall':'UTF-8'}" placeholder="{$input.label|escape:'htmlall':'UTF-8'}" id="{$input.name|escape:'htmlall':'UTF-8'}" name="{$input.name|escape:'htmlall':'UTF-8'}" value="{$input.value|escape:'htmlall':'UTF-8'}"/></li>
		...
		{elseif $input.type == 'radio'}
			<ul>
				...
				<li><input type="submit" class="md-btn button-form_pagomio button_payu" value="{l s='Save' mod='pagomio'}" /></li>
			</ul>
		{/if}
	{/foreach}
</form>
